...
class IntcodeComputer:
    ...
    def run(self, input_signal, start_position=None):
        ...
        while True:
            ...
            try:
                pos1 = self.data[self.data[self.position + 1]
                                 ] if mode_1 == 0 else self.data[self.position + 1]
                pos2 = self.data[self.data[self.position + 2]
                                 ] if mode_2 == 0 else self.data[self.position + 2]
            ...
            if instruction == 1:  # Add
                ...
                self.data[self.data[self.position + 3]
                          ] = 1 if pos1 < pos2 else 0
                self.position += 4
...
if __name__ == "__main__":
    ...
    assert amplifiers([3, 23, 3, 24, 1002, 24, 10, 24, 1002, 23, -1, 23, 101,
                       5, 23, 23, 1, 24, 23, 23, 4, 23, 99, 0, 0], [0, 1, 2, 3, 4]) == 54321
    assert amplifiers([3, 31, 3, 32, 1002, 32, 10, 32, 1001, 31, -2, 31, 1007, 31, 0, 33, 1002,
                       33, 7, 33, 1, 33, 31, 31, 1, 32, 31, 31, 4, 31, 99, 0, 0, 0], [1, 0, 4, 3, 2]) == 65210

    assert part1(
        [3, 15, 3, 16, 1002, 16, 10, 16, 1, 16, 15, 15, 4, 15, 99, 0, 0]
    ) == 43210
    assert part1([3, 23, 3, 24, 1002, 24, 10, 24, 1002, 23, -1,
                  23, 101, 5, 23, 23, 1, 24, 23, 23, 4, 23, 99, 0, 0]) == 54321
    assert part1([3, 31, 3, 32, 1002, 32, 10, 32, 1001, 31, -2, 31, 1007, 31, 0, 33,
                  1002, 33, 7, 33, 1, 33, 31, 31, 1, 32, 31, 31, 4, 31, 99, 0, 0, 0]) == 65210
    print(part1(readFile("input")), "should be", 101490)

    print("Part2")
    assert amplifiers_p2([3, 26, 1001, 26, -
                          4, 26, 3, 27, 1002, 27, 2, 27, 1, 27, 26, 27, 4, 27, 1001, 28, -
