...
public class RobotMap {
  ...
  private boolean isWalkable(Position position) {
    ...
    if (position.getX() < 0
        || position.getX() >= map2d[0].length - 1
        || position.getY() < 0
        || position.getY() >= map2d.length - 1) {
      return false;
    }
    ...
    while (!positionsToVisit.isEmpty()) {
      ...
      if (!visitedPositions.contains(currentPosition)) {
        ...
        if (pointsOfInterest.contains("" + getChar(currentPosition))) {
          // System.out.println(start.getChar() + " -> " + currentPosition.getChar() + " is " +
        ...
        Position[] adjacentPositions =
            new Position[] {
              new Position(
                  ...
                  currentPosition.getDistance() + 1), // Above
              new Position(
                  currentPosition.getX(),
                  currentPosition.getY() - 1,
                  currentPosition.getDistance() + 1), // Below
              new Position(
                  currentPosition.getX() + 1,
                  currentPosition.getY(),
                  currentPosition.getDistance() + 1), // Left
              new Position(
                  currentPosition.getX() - 1,
                  currentPosition.getY(),
                  currentPosition.getDistance() + 1) // Right
            };
